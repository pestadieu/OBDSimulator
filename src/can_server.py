...
class Simulation():
	...
	def setSpeed(self, speed):
		set_value(0x01, 0x0D, speed.encode('iso-8859-15'))

	def setRPM(self, rpm):
		set_value(0x01, 0x0C, rpm.encode('iso-8859-15'))

	def setFuelPressure(self, fuel_pressure):
		set_value(0x01, 0x0A, fuel_pressure.encode('iso-8859-15'))

	def setCoolantTemperature(self, coolant_temperature):
		set_value(0x01, 0x05, coolant_temperature.encode('iso-8859-15'))
	...
	def updateSpeed(self):
		...
		if currentTime - self.startTime > self.speed[self.countUpdateSpeed]["time"]:
			...
			self.setSpeed( str(self.speed[self.countUpdateSpeed]["value"]) )
	...
	def updateRPM(self):
		...
		if currentTime - self.startTime > self.rpm[self.countUpdateRPM]["time"]:
			...
			self.setRPM( str(self.rpm[self.countUpdateRPM]["value"]) )
	...
	def updateFuelPressure(self):
		...
		if currentTime - self.startTime > self.fuel_pressure[self.countUpdateFuelPressure]["time"]:
			...
			self.setFuelPressure( str(self.fuel_pressure[self.countUpdateFuelPressure]["value"]) )
	...
	def updateCoolantTemperature(self):
		...
		if currentTime - self.startTime > self.coolant_temperature[self.countUpdateCoolantTemperature]["time"]:
			...
			self.setCoolantTemperature( str(self.coolant_temperature[self.countUpdateCoolantTemperature]["value"]) )
